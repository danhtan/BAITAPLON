...
<body>
	<div style="margin-left: 20px ;background-color: white;padding: 40px; margin-top: 20px" >	
		<button  style="float: left ;margin: 10px ; padding: 10px; background-color: blue;color: white" class="btn btn-primary haygap">Những Câu Hỏi Hay gặp</button>
       ...
       <!-- <button type="button" name="age" id="age" data-toggle="modal" data-target="#add_data_Modal" class="btn btn-warning"><i class="fa fa-plus-circle" style="font-size:16px ; margin-right: 3px"></i>Add</button>  -->
		<button  style="float: left ;  margin: 10px ; padding: 10px; background-color: blue;color: white" class="btn btn-primary danhmuc">Danh Mục HỎi Đáp</button> 
		<button  style="float: left ;  margin: 10px; padding: 10px; background-color: blue;color: white" class="btn btn-primary themcauhoi"  data-toggle="modal" data-target="#add_data_Modal">Thêm Câu Hỏi</button>
		<br>
  ...
  <div class="modal-content" >
   ...
   <div class="modal-body">
    <form method="POST" id="insert_form" >
        ...
        <?php
           include("../config.php");
        $sql="select *from hoidap  order by mach DESC limit 0,1";
        $kq=mysqli_query($conn,$sql);
        $row=mysqli_fetch_array($kq);
        ?>
     <input type="text" name="mach" id="mach" class="form-control" value="<?php echo $row['mach']+1 ?>" />
     <br />
     <label>Câu Hỏi</label>
...
<script type="text/javascript">
 ...
 $('#insert_form').on("submit", function(event){  
  ...
  if($('#mach').val() == "")  
  {  
   alert("Mã Câu Hỏi Không đk để trống");  
  }  
  else if($('#cauhoi').val() == '')  
  {

   alert("Câu Hỏi Không đk để trống");  
  }  
  ...
  $.ajax({
   url:"block2/hoidap/danhsachcauhoi.php",
   method:"POST",
   success:function(data){
   ...
   error: function(){
    alert('error!');
     ...
     $(document).on('click', '.haygap', function(){

  $.ajax({
   url:"block2/hoidap/cauhoithuonggap.php",
   method:"POST",
   success:function(data){
